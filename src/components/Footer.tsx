import React from 'react';
import { Heart } from 'lucide-react';

const Footer = () => {
  return (
    <footer className="bg-background py-24 border-t border-primary/20 relative overflow-hidden">
      <div className="max-w-4xl mx-auto px-4 text-center relative z-10">
        <div className="mb-10">
          <h3 className="font-serif text-2xl xs:text-3xl sm:text-4xl md:text-5xl font-bold mb-4 text-foreground tracking-wide whitespace-nowrap">
            M. V. Chiranjith <span className="text-primary italic font-light">&</span> K. Thriveni
          </h3>
          <div className="text-lg font-serif italic text-primary">
            Together, Always
          </div>
          <div className="text-sm font-sans font-medium text-foreground/60 tracking-widest uppercase mt-2">
            23rd August 2026
          </div>
        </div>

        <div className="w-16 h-[1.5px] bg-primary/30 mx-auto my-8"></div>

        <div className="max-w-2xl mx-auto mb-10">
          <p className="mb-6 text-base sm:text-lg font-sans font-light leading-relaxed text-foreground/80">
            With the blessings of our families and ancestors, we take our first steps into a life of togetherness. Bound in heart, spirit, and purpose, we vow to walk side by side, sharing a journey rooted in trust, unity, and devotion.
          </p>
          <p className="text-base font-serif font-medium text-primary italic">
            We look forward to celebrating this special day with you.
          </p>
        </div>

        <div className="w-16 h-[1.5px] bg-primary/30 mx-auto my-8"></div>

        <div className="mt-8 text-center">
          <p className="text-xs font-sans text-foreground/50 tracking-widest uppercase flex items-center justify-center">
            Crafted with <Heart size={12} className="mx-1.5 text-primary" /> for Chiranjith & K. Thriveni's Wedding
          </p>
        </div>
      </div>
    </footer>
  );
};

export default Footer;
